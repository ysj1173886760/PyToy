...
class Node(object):
    ...
    def backward(self, target):
        ...
        if self.graident is None:
            self.graident = np.zeros_like(self.value)
            for node in self.children:
                if node.value is not None:
                    if node.graident is None:
                        node.backward(target)
                    self.graident = self.graident + node.get_graident(self)
...
class Variable(Node):
    ...
    def __init__(self, dims, init=False, trainable=False, **kargs) -> None:
        ...
        if init:
            ...
            if is_bias:
                self.value = np.zeros(dims, dtype=np.int32)
            else:
                mean = kargs.get('mean', 0.0)
                std = kargs.get('std', 0.001)
                self.value = np.random.normal(loc=mean, scale=std, size=dims)
